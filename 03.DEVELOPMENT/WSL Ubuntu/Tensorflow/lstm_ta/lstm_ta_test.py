...
folderPath_Logs ="/Users/Shared/ai_work/Trainingdata/ml_logs/"
...
def createJobFile (jobnamefile, st_start, description):
...
#======SETTINGS FB10 ==============
jobfile_analysis  = "trend_lstm_ta.txt" 
st_job = "forecast_lstm_ta.txt"
uploadfile ="upload_lstm_ta.txt"

checksymbol ="" # Null > check all; Not null: check only the symbol. 
filePath =folderPath_Jobs + st_job
#--- END SETTINGS FB10 ------------
file_exists = os.path.isfile(filePath)  
if file_exists == False: 
   print ("No Job file for sk_linear")
   SystemExit()
if  file_exists  == True:
 
    df_log = createTrainLog()

    st_start  = str(datetime.now()) 
    #deleleteCreateFolder (folder_path)
    #deleteOldFile(folder_path)
    ...
    for entry in listOfFiles:
        ...
        if i_run == 1: 
          ...
          try: 
            ...
            print ("Running LSTM TA  " , symbol)
            predictBySymbol (symbol)
           
            
          except:
             print ("Error LSTM TA  " , symbol)
             
             log_row = {'Symbol': symbol, 'Job': "lstm_ta", 'Date': str(datetime.now()), 'Status': "FAILED", 'Comment': "Error during LSTM TA processing"}
             df_log = pd.concat([df_log, pd.DataFrame([log_row])], ignore_index=True)


    
    df_log.to_csv ( folderPath_Logs + "lstm_ta_training_log.csv", index=False)
    createJobFile ("trend/"+jobfile_analysis , st_start, "OK")
